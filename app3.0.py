...
專家增強最終版實作 (V8.1 - Guided Charting)
...
核心功能：
- [專家圖表 Pro] 為指標圖表加入關鍵水平線與 AI 註解，引導使用者解讀
- [UI 優化] 調整 AI 專家總結的文字格式
- [穩定性強化] 提高數據驗證門檻
- [專家升級] 引入動態適應功能
...
def generate_ai_fusion_signal(df, funda_scores, inst_hold_pct):
    ...
    rsi = latest['RSI_9']
    if rsi < 30: momentum_score, opinions['動能分析 (RSI 9)'] = 2.0, f"<span class='positive'>超賣 ({rsi:.1f})</span>"
    elif rsi > 70: momentum_score, opinions['動能分析 (RSI 9)'] = -2.0, f"<span class='negative'>超買 ({rsi:.1f})</span>"
...
def generate_expert_summary(signal, risk, df, name):
    ...
    if "買" in action: summary = f"📈 「AI 觀點：目前 「{name}」 技術面偏多，由 「{driver}」 帶動。市場「{condition}」，建議 「{action}」。共識支撐 (SL) 約在 「{risk['sl']:.2f}」，壓力 (TP) 約在 「{risk['tp']:.2f}」。」"
    elif "賣" in action: summary = f"📉 「AI 觀點：目前 「{name}」 技術面偏空，由 「{driver}」 帶動。市場「{condition}」，建議 「{action}」。共識壓力 (SL) 約在 「{risk['sl']:.2f}」，支撐 (TP) 約在 「{risk['tp']:.2f}」。」"
    else: summary = f"📊 「AI 觀點：目前 「{name}」 處於中性觀望。市場「{condition}」，建議 「{action}」。關鍵支撐與壓力位於 「{risk['sl']:.2f}」 和 「{risk['tp']:.2f}」。」"
    return f"<div class='expert-summary'>{summary}</div>"

def get_technical_data_df(df):
    if df.empty: return pd.DataFrame(); latest = df.iloc[-1]; data = []
    ...
    rsi = latest['RSI_9']; data.append(['RSI (9)', f"{rsi:.2f}", f"<span class='negative'>超買</span>" if rsi > 70 else (f"<span class='positive'>超賣</span>" if rsi < 30 else '中性')])
...
with st.sidebar:
    ...
    st.subheader("3. 選擇週期"); selected_period_label = st.selectbox("", list(PERIOD_MAP.keys()), index=2, label_visibility="collapsed"); period, interval = PERIOD_MAP[selected_period_label]
    st.markdown("---"); st.subheader("4. 開始分析"); start_analysis = st.button("📊 執行AI分析", use_container_width=True)
        ...
        if data is not None and len(data) > 55:
            ...
            if df.empty or len(df) < 2:
                ...
                fig = make_subplots(rows=4, cols=1, shared_xaxes=True, vertical_spacing=0.04, row_heights=[0.55, 0.15, 0.15, 0.15],
                                    subplot_titles=("<b>價格與趨勢</b>", "<b>核心動能 (RSI / MACD)</b><br><sup><i>觀察指標是否進入 70 以上(超買) 或 30 以下(超賣) 的極端區域</i></sup>",
                                                    "<b>趨勢強度與資金流 (ADX / CMF)</b><br><sup><i>ADX > 25 代表趨勢增強，CMF > 0 代表資金流入</i></sup>", "<b>擺盪指標 (Stochastic %K)</b><br><sup><i>觀察指標是否進入 80 以上(超買) 或 20 以下(超賣) 的區域</i></sup>"))
                ...
                fig.add_trace(go.Bar(x=df.index, y=df['CMF'], name='CMF', marker_color=np.where(df['CMF'] DRAFTING...
