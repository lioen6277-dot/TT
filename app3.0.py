    ...
    with st.spinner(f'AI 分析引擎啟動中，正在分析 {final_symbol_name}...'):
        ...
        if data is not None and len(data) > 50:
            ...
            if df.empty:
                ...
                # 布林通道
                fig.add_trace(go.Scatter(x=df.index, y=df['BB_High'], name='BB 上軌', line=dict(color='cyan', width=1, dash='dash')), row=1, col=1)
                fig.add_trace(go.Scatter(x=df.index, y=df['BB_Low'], name='BB 下軌', line=dict(color='cyan', width=1, dash='dash'), fill='tonexty', fillcolor='rgba(0, 255, 255, 0.1)'), row=1, col=1)
                # 一目均衡表 (雲區)
                fig.add_trace(go.Scatter(x=df.index, y=df['Ichimoku_A'], name='Ichimoku A', line=dict(color='rgba(40, 167, 69, 0.5)', width=1)), row=1, col=1)
                fig.add_trace(go.Scatter(x=df.index, y=df['Ichimoku_B'], name='Ichimoku B', line=dict(color='rgba(220, 53, 69, 0.5)', width=1), fill='tonexty', fillcolor='rgba(128, 128, 128, 0.2)'), row=1, col=1)
                
                # --- 第 2 列: RSI ---
                fig.add_trace(go.Scatter(x=df.index, y=df['RSI_9'], name='RSI', line=dict(color='purple', width=1.5)), row=2, col=1)
                ...
                # --- 第 3 列: Stochastics %K ---
                fig.add_trace(go.Scatter(x=df.index, y=df['Stoch_%K'], name='Stoch %K', line=dict(color='orange', width=1.5)), row=3, col=1)
                ...
                # --- 第 4 列: CMF ---
                colors_cmf = np.where(df['CMF'] > 0, '#28a745', '#dc3545')
                fig.add_trace(go.Bar(x=df.index, y=df['CMF'], name='CMF', marker_color=colors_cmf), row=4, col=1)
                
                # --- 第 5 列: ADX ---
                fig.add_trace(go.Scatter(x=df.index, y=df['ADX_9'], name='ADX', line=dict(color='brown', width=1.5)), row=5, col=1)
                fig.add_hline(y=25, line_width=1, line_dash="dash", line_color="grey", row=5, col=1)

                # --- 圖表整體布局 ---
                fig.update_layout(
                    title_text="完整技術指標分析圖",
                    height=1000, 
                ...
                fig.update_yaxes(title_text="Stoch %K", range=[0, 100], row=3, col=1)
