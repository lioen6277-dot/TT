...
專家增強最終版實作 (V8.2 - RSI 80/20 Strategy)
...
核心功能：
- [策略升級] 將 RSI 超買/超賣閾值統一調整為 80/20，過濾市場雜訊
...
def get_technical_data_df(df):
    ...
    rsi = latest['RSI_9']
    data.append(['RSI (9)', f"{rsi:.2f}", f"<span class='negative'>超買</span>" if rsi > 80 else (f"<span class='positive'>超賣</span>" if rsi < 20 else '中性')])
    adx = latest['ADX_9']
    data.append(['ADX (9)', f"{adx:.2f}", f"<span class='positive'>強趨勢</span>" if adx > 25 else '盤整'])
        ...
        if data is not None and len(data) > 55:
            ...
            if df.empty or len(df) < 2: st.error("錯誤：數據清洗後不足以進行分析，請嘗試更長的分析週期。")
                ...
                fig = make_subplots(rows=4, cols=1, shared_xaxes=True, vertical_spacing=0.04, row_heights=[0.55, 0.15, 0.15, 0.15],
                                    subplot_titles=("<b>價格與趨勢</b>", "<b>核心動能 (RSI / MACD)</b><br><sup><i>觀察 RSI 是否進入 80 以上(超買) 或 20 以下(超賣) 的極端區域</i></sup>",
