...
專家增強最終版實作 (V7.6 - Stability Patch)
...
def calculate_indicators(df):
    ...
    # [穩定性修復] 增加 SuperTrend 計算的 Failsafe
    try:
        ...
        st.warning(f"無法計算 SuperTrend 指標: {e}")
        df['SuperTrend'] = np.nan
        df['SuperTrend_Direction'] = 0
...
def generate_ai_fusion_signal(df, funda_scores, inst_hold_pct):
    ...
    if adx_value > 25:
        strength_score *= 1.5
        opinions['趨勢強度 (ADX 9)'] = f"<span class='positive'>確認強趨勢: ADX {adx_value:.1f} > 25</span>"
    else:
    ...
    if adx_value < 20:
        market_regime_filter = 0.6 # 在盤整市中，降低總分權重
...
def get_individual_stops(df):
    ...
    if 'SuperTrend' in df.columns and pd.notna(latest['SuperTrend']):
        if latest['SuperTrend_Direction'] == 1:
            stops['sl'].append(latest['SuperTrend'])
        else:
            stops['tp'].append(latest['SuperTrend'])
...
def run_backtest(df, initial_capital=100000):
    ...
    for i in range(len(df)):
        if df['signal'].iloc[i] == 1 and position_size == 0:
            ...
            trades.append(cash - (position_size * entry_price))
            position_size = 0
    ...
    with st.spinner(f'AI 分析引擎啟動中，正在分析 {final_symbol_name}...'):
        ...
        if data is not None and len(data) > 50:
            ...
            if df.empty:
                ...
                cmf_colors = np.where(df['CMF'] > 0, '#28a745', '#dc3545')
