...
專家增強最終版實作 (V7.5 - Expert Advisor Integration)
...
def get_individual_stops(df):
    ...
    # 策略 3: 超級趨勢
    if 'SuperTrend' in df.columns and latest['SuperTrend_Direction'] == 1:
...
def generate_expert_summary(signal, risk, df, symbol_name):
    ...
    # 判斷主要驅動因子
    main_driver = "趨勢與動能指標"
    if "RSI" in opinions.get('動能分析 (RSI 9)', ''):
        main_driver = f"**RSI ({df.iloc[-1]['RSI_9']:.1f})**"
    if "MACD" in opinions.get('趨勢強度 (MACD)', ''):
        main_driver = f"**MACD** 與 **RSI ({df.iloc[-1]['RSI_9']:.1f})**"
    ...
    with st.spinner(f'AI 分析引擎啟動中，正在分析 {final_symbol_name}...'):
        ...
        if data is not None and len(data) > 50:
            ...
            if df.empty:
                ...
                if (risk['entry'] - risk['sl']) != 0:
                    risk_reward_ratio = abs((risk['tp'] - risk['entry']) / (risk['entry'] - risk['sl']))
                    rr_text = f"{risk_reward_ratio:.2f}"
                else:
                    rr_text = "N/A"
                ...
                fig = make_subplots(
                    ...
                    vertical_spacing=0.06, row_heights=[0.6, 0.2, 0.2],
                    subplot_titles=("價格走勢", "動能指標 (MACD / RSI)", "資金流與擺盪指標 (CMF / Stoch)"),
                    specs=[[{"secondary_y": True}], [{"secondary_y": True}], [{"secondary_y": True}]]
                )
                
                # --- Row 1: Price, Volume, EMAs, Bollinger Bands, Ichimoku Cloud ---
                ...
                fig.add_trace(go.Scatter(x=df.index, y=df['Ichimoku_B'], name='Ichimoku B', line=dict(color='rgba(220, 53, 69, 0.5)', width=1.5), 
                                         fill='tonexty', fillcolor='rgba(150, 100, 100, 0.1)'), row=1, col=1)
                ...
                # --- Row 3: Stochastic %K, Chaikin Money Flow (CMF) ---
