...
專家增強最終版實作 (V9.0 - Professional Strategy Integration)
...
版本：9.0.0 (Professional Strategy)
"""
...
ANALYSIS_HORIZONS = {
    ...
    "中線": {'15 分': ('30d', '15m'), '1 小時': ('60d', '60m'), '4 小時': ('1y', '60m')}, # Note: 4h uses 60m interval
...
@st.cache_data(ttl=300) # Reduced TTL for shorter timeframes
...
with st.sidebar:
    ...
    st.subheader("3. 選擇分析視野")
    selected_horizon = st.selectbox("", list(ANALYSIS_HORIZONS.keys()), index=1, label_visibility="collapsed") # Default to "中線"
    
    st.subheader("4. 選擇分析時間週期")
    selected_period_label = st.selectbox("", list(ANALYSIS_HORIZONS[selected_horizon].keys()), label_visibility="collapsed")
