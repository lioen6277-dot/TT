...
專家增強最終版實作 (V5.1 - Backtest Engine Fix & Upgrade)
...
版本：5.1.0 (Backtest Engine Fix & Upgrade)
"""
...
# (FIXED & UPGRADED v5.1) 修正 ValueError 並升級為 Mark-to-Market 模型
def run_backtest(df, initial_capital=100000):
    ...
    # 產生交易信號
    df['signal'] = 0
    df.loc[(df['SMA_20'] > df['EMA_50']) & (df['SMA_20'].shift(1) <= df['EMA_50'].shift(1)), 'signal'] = 1  # Buy
    df.loc[(df['SMA_20'] < df['EMA_50']) & (df['SMA_20'].shift(1) >= df['EMA_50'].shift(1)), 'signal'] = -1 # Sell
    ...
    with st.spinner(f'AI 分析引擎啟動中...'):
        ...
        if data is not None and len(data) > 50:
            ...
            if df.empty:
                ...
                # 從 session state (如果存在) 或 UI 輸入獲取 risk_per_trade
                # risk_per_trade = st.session_state.get('risk_per_trade', 0.02) # 假設 2%
                # total_capital = st.session_state.get('total_capital', 100000) # 假設 100000
