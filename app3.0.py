...
專家增強最終版實作 (V5.0 - Pixel-Perfect UI Refactor)
...
版本：5.0.0 (Pixel-Perfect UI Refactor)
"""
...
def run_backtest(df, initial_capital=100000):
    ...
    for i in range(1, len(df)):
        current_capital = capital_over_time[-1]
        if df['signal'].iloc[i] == 1 and position == 0:
            position = current_capital / df['Close'].iloc[i]
    ...
    if not trades: return {"total_return": 0, "win_rate": 0, "max_drawdown": 0, "total_trades": 0, "equity_curve": pd.Series([initial_capital]*len(df))}

    equity_curve = pd.Series(capital_over_time, index=df.index[1:])
    ...
    with st.spinner(f'AI 分析引擎啟動中...'):
        ...
        if data is not None and len(data) > 50:
            ...
            if df.empty:
                ...
                pos_size = int((total_capital * risk_per_trade) / (risk['entry'] - risk['sl'])) if risk['entry'] > risk['sl'] else "N/A"
                ...
                fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.05, row_heights=[0.6, 0.2, 0.2],
                                    subplot_titles=("價格走勢", "MACD 指標", "RSI/ADX 指標"))
                ...
                fig.add_trace(go.Scatter(x=df.index, y=df['ADX_9'], name='ADX', line=dict(color='brown', dash='dot'), yaxis="y2"), row=3, col=1)
                fig.update_layout(height=700, xaxis_rangeslider_visible=False, template="plotly_dark",
                                  yaxis2=dict(title="ADX", overlaying="y", side="right", range=[0, 100], showgrid=False),
