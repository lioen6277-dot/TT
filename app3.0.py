...
專家增強最終版實作 (V11.1 - Final Stable Version)
...
def run_multi_timeframe_analysis(symbol, name, strategy):
    ...
    st.markdown(f"""<div class="metric-container">
        ...
        <div class="metric-card"><div class="metric-label">當前價格 ({tf_short})</div><div class="metric-value" style="color: #FA8072; font-weight: bold;">~{df_short.iloc[-1]['Close']:,.2f}</div></div>
    ...
    </div><div class="strategy-summary">💡 <b>策略總結:</b> 基於多週期共振信號，在 <b>~{df_short.iloc[-1]['Close']:,f}</b> 範圍尋找機會 | ⚖️ <b>R:R:</b> {rr if isinstance(rr, str) else f'{rr:.2f}'}</div>""", unsafe_allow_html=True)
