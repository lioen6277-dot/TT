...
專家增強最終版實作 (V10.1 - Final Polished Version)
...
def run_multi_timeframe_analysis(symbol, name):
    df_4h, df_1h, df_15m = get_data(symbol, '1y', '4h'), get_data(symbol, '60d', '60m'), get_data(symbol, '30d', '15m')
    ...
    df_4h, df_1h, df_15m = calculate_indicators(df_4h.copy()), calculate_indicators(df_1h.copy()), calculate_indicators(df_15m.copy())
    trend_4h = "Up" if df_4h.iloc[-1]['EMA_50'] > df_4h.iloc[-1].get('EMA_200', 0) and df_4h.iloc[-1]['ADX_9'] > 20 else "Down" if df_4h.iloc[-1]['EMA_50'] < df_4h.iloc[-1].get('EMA_200', 0) and df_4h.iloc[-1]['ADX_9'] > 20 else "Sideways"
    ...
    else: summary = f"📊 「AI 觀點：目前在 4h/1h/15m 的多週期框架下，未發現明顯的共振交易信號。主要趨勢（4h）為「{trend_4h}」，但中短期訊號不一致。建議「<span class='neutral'>{final_action}</span>」，等待更清晰的結構。」"
