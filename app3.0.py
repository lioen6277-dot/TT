# -*- coding: utf-8 -*-
"""
AI 趨勢分析 Streamlit 應用程式
專家增強最終版實作 (V12.2 - Final Stable Version)

本應用程式根據一份詳細的金融分析工具設定文件進行開發，並融合了專業級的
app2.0.py 設計 logique 與使用者提供的 UI 視覺設計稿，旨在提供一個外觀精美、
互動專業的決策儀表板。

核心功能：
- [穩定性修復] 新增 scipy 依賴，修復 ModuleNotFoundError
- [穩定性修復] 修復 UnboundLocalError 並恢復所有儀表板區塊
- [頂級策略] 引入「多週期趨勢確認」分析引擎
- [頂級策略] 整合「斐波那契回測」作為動態止盈止損模型
- [策略升級] 更新 MACD 核心參數為 (9, 16, 5)
- [UI 優化] 為 AI 專家總結的行動建議添加顏色標記
- [策略升級] 將 RSI 超買/超賣閾值統一調整為 80/20
- [專家圖表 Pro] 為指標圖表加入關鍵水平線與 AI 註解
- [穩定性強化] 提高數據驗證門檻
- [專家升級] 引入動態適應功能

開發者：程式碼專家 (Generated by Gemini)
版本：12.3.0 (UI Optimized)
"""

# 載入核心套件
import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from plotly.subplots import make_subplots
import ta
import warnings
import time
import re 
from datetime import datetime, timedelta
# 新增 scipy 依賴，確保穩定性
try:
    from scipy.stats import linregress
except ImportError:
    st.error("錯誤: 缺少必要的 'scipy' 套件。請執行 'pip install scipy' 安裝。")
    st.stop()

warnings.filterwarnings('ignore')

# ==============================================================================
# 1. 頁面配置與全局設定
# ==============================================================================

st.set_page_config(
    page_title="AI趨勢分析 V12.3", 
    page_icon="📈", 
    layout="wide"
)

# 週期映射：(YFinance Period, YFinance Interval)
PERIOD_MAP = { 
    "30 分": ("60d", "30m"), 
    "4 小時": ("1y", "60m"), 
    "1 日": ("5y", "1d"), 
    "1 週": ("max", "1wk")
}

# 🚀 您的【所有資產清單】 (簡化，僅為範例)
FULL_SYMBOLS_MAP = {
    # ----------------------------------------------------
    # A. 美股核心 (US Stocks) - 個股
    # ----------------------------------------------------
    "TSLA": {"name": "特斯拉", "keywords": ["特斯拉", "電動車", "TSLA", "Tesla"]},
    "NVDA": {"name": "輝達", "keywords": ["輝達", "英偉達", "AI", "NVDA", "Nvidia"]},
    "AAPL": {"name": "蘋果", "keywords": ["蘋果", "手機", "AAPL", "Apple"]},
    # ----------------------------------------------------
    # B. 台股核心 (TW Stocks) - 個股
    # ----------------------------------------------------
    "2330.TW": {"name": "台積電", "keywords": ["台積電", "晶圓", "2330", "TSMC"]},
    "2454.TW": {"name": "聯發科", "keywords": ["聯發科", "MTK", "2454"]},
    # ----------------------------------------------------
    # C. 加密貨幣 (Crypto)
    # ----------------------------------------------------
    "BTC-USD": {"name": "比特幣/USD", "keywords": ["比特幣", "BTC", "Bitcoin"]},
    "ETH-USD": {"name": "以太幣/USD", "keywords": ["以太幣", "ETH", "Ethereum"]},
}


# ==============================================================================
# 2. 數據獲取與處理
# ==============================================================================

@st.cache_data(ttl=3600, show_spinner="🚀 正在從 Yahoo Finance 獲取數據...")
def get_historical_data(symbol, period, interval):
    """
    從 yfinance 獲取歷史數據。
    """
    try:
        data = yf.download(symbol, period=period, interval=interval, progress=False)
        if data.empty or len(data) < 10:
            return pd.DataFrame(), f"錯誤：無法獲取 {symbol} 的數據，或數據量不足 (僅有 {len(data)} 根K線)。"
        
        # 數據驗證門檻提高：至少需要 50 根K線才能進行有效的技術分析
        if len(data) < 50:
             return pd.DataFrame(), f"數據量不足：需要至少 50 根K線進行有效分析，目前僅有 {len(data)} 根。"

        # 確保列名規範化 (大寫)
        data.columns = [col.capitalize() for col in data.columns]
        data.index.name = 'Date'
        return data, None
    except Exception as e:
        return pd.DataFrame(), f"數據獲取異常: {e}"

def calculate_indicators(df, lookback_candles):
    """
    計算一系列技術指標，並根據 lookback_candles 裁切數據。
    """
    if df.empty:
        return df

    # 1. 趨勢指標
    df['SMA_20'] = df['Close'].rolling(window=20).mean()
    df['EMA_50'] = df['Close'].ewm(span=50, adjust=False).mean()
    df['MACD_Line'] = ta.trend.macd_diff(df['Close'], window_fast=9, window_slow=16, window_sign=5)
    df['MACD_Signal'] = ta.trend.macd_signal(df['Close'], window_fast=9, window_slow=16, window_sign=5)
    df['MACD_Hist'] = ta.trend.macd_diff(df['Close'], window_fast=9, window_slow=16, window_sign=5)
    df['ADX_9'] = ta.trend.adx(df['High'], df['Low'], df['Close'], window=9)
    df['ADX_pos'] = ta.trend.adx_pos(df['High'], df['Low'], df['Close'], window=9)
    df['ADX_neg'] = ta.trend.adx_neg(df['High'], df['Low'], df['Close'], window=9)

    # 2. 動量指標
    df['RSI'] = ta.momentum.rsi(df['Close'], window=14)
    df['Stoch_%K'] = ta.momentum.stoch(df['High'], df['Low'], df['Close'], window=14, smooth_window=3)
    df['Stoch_%D'] = ta.momentum.stoch_signal(df['High'], df['Low'], df['Close'], window=14, smooth_window=3)

    # 3. 成交量指標
    df['CMF'] = ta.volume.money_flow_index(df['High'], df['Low'], df['Close'], df['Volume'], window=20, fillna=False)
    
    # 4. 波動性指標 (布林帶)
    bb = ta.volatility.BollingerBands(close=df['Close'], window=20, window_dev=2)
    df['BB_High'] = bb.bollinger_hband()
    df['BB_Low'] = bb.bollinger_lband()
    df['BB_Mid'] = bb.bollinger_mavg()
    
    # 5. 斐波那契回測 (動態止盈止損模型)
    # 使用最後 lookback_candles 內的最高和最低價
    recent_df = df.iloc[-lookback_candles:] 
    high_250 = recent_df['High'].max()
    low_250 = recent_df['Low'].min()
    diff = high_250 - low_250
    df['FIB_R3'] = high_250 - 0.236 * diff
    df['FIB_R2'] = high_250 - 0.382 * diff
    df['FIB_R1'] = high_250 - 0.500 * diff
    df['FIB_S1'] = low_250 + 0.500 * diff
    df['FIB_S2'] = low_250 + 0.382 * diff
    df['FIB_S3'] = low_250 + 0.236 * diff


    # 6. 裁切數據: 只保留 lookback_candles 數量的數據
    df = df.iloc[-lookback_candles:]
    df = df.dropna()

    return df

def get_multi_timeframe_trend(df_daily, df_weekly):
    """
    多週期趨勢確認：檢查日線和週線趨勢。
    """
    if df_daily.empty or df_weekly.empty:
        return "數據不足", 0

    # 1. 日線趨勢 (Daily Trend)
    # 使用最後一根日K線的 MACD 柱狀圖和 ADX 強度
    daily_macd_hist = df_daily['MACD_Hist'].iloc[-1]
    daily_adx = df_daily['ADX_9'].iloc[-1]
    
    # 趨勢強度判定
    daily_strength = "強" if daily_adx > 35 else ("中" if daily_adx > 25 else "弱")

    if daily_macd_hist > 0:
        daily_trend = f"日線：{daily_strength}勢上漲"
        daily_score = 1
    elif daily_macd_hist < 0:
        daily_trend = f"日線：{daily_strength}勢下跌"
        daily_score = -1
    else:
        daily_trend = "日線：盤整"
        daily_score = 0

    # 2. 週線趨勢 (Weekly Trend)
    # 使用最後一根週K線的 MACD 柱狀圖
    weekly_macd_hist = df_weekly['MACD_Hist'].iloc[-1]

    if weekly_macd_hist > 0:
        weekly_trend = "週線：多頭排列"
        weekly_score = 1
    elif weekly_macd_hist < 0:
        weekly_trend = "週線：空頭排列"
        weekly_score = -1
    else:
        weekly_trend = "週線：盤整"
        weekly_score = 0

    # 3. 綜合得分 (Composite Score)
    total_score = daily_score + weekly_score
    
    # 4. 趨勢確認結果
    if total_score == 2:
        confirmation = "<span style='color: #28a745; font-weight: bold;'>🟢 強烈多頭確認 (雙重趨勢向上)</span>"
    elif total_score == -2:
        confirmation = "<span style='color: #dc3545; font-weight: bold;'>🔴 強烈空頭確認 (雙重趨勢向下)</span>"
    elif total_score == 1:
        confirmation = "<span style='color: #ffc107;'>🟡 潛在多頭 (日線強於週線)</span>"
    elif total_score == -1:
        confirmation = "<span style='color: #ffc107;'>🟡 潛在空頭 (週線強於日線)</span>"
    else:
        confirmation = "⚪ 趨勢不明或盤整"
        
    summary = f"{confirmation}<br><small>{daily_trend}, {weekly_trend}</small>"
    
    return summary, total_score


def generate_ai_analysis(df, symbol_name, total_score):
    """
    根據技術指標和多週期確認結果生成 AI 交易策略。
    """
    if df.empty:
        return "數據不足，無法生成分析。", "中性"

    last_close = df['Close'].iloc[-1]
    
    # --- 指標狀態判定 ---
    macd_status = "多頭" if df['MACD_Hist'].iloc[-1] > 0 else "空頭"
    rsi_status = "超買" if df['RSI'].iloc[-1] > 80 else ("超賣" if df['RSI'].iloc[-1] < 20 else "中性")
    stoch_status = "超買" if df['Stoch_%K'].iloc[-1] > 80 else ("超賣" if df['Stoch_%K'].iloc[-1] < 20 else "中性")
    adx_status = "強勢" if df['ADX_9'].iloc[-1] > 25 else "弱勢/盤整"
    cmf_status = "資金流入" if df['CMF'].iloc[-1] > 0 else "資金流出"
    
    # 布林帶突破判定
    bb_breakout = ""
    if last_close > df['BB_High'].iloc[-1]:
        bb_breakout = "突破上軌 (強勢)"
    elif last_close < df['BB_Low'].iloc[-1]:
        bb_breakout = "跌破下軌 (弱勢)"

    # --- 綜合策略生成 ---
    if total_score >= 1.5: # 強烈多頭確認
        action = "🟢 強烈買入 (Strong Buy)"
        reason = f"多週期強烈多頭確認。MACD (9, 16, 5) 柱狀線持續為正 ({macd_status})，ADX 顯示強勢趨勢 ({adx_status})。當前價格 {last_close:.2f} 處於上升通道，且資金流向 ({cmf_status}) 偏向正面。"
        risk_level = "中高"
    elif total_score <= -1.5: # 強烈空頭確認
        action = "🔴 強烈賣出 (Strong Sell)"
        reason = f"多週期強烈空頭確認。MACD 柱狀線持續為負 ({macd_status})，ADX 顯示趨勢強勁 ({adx_status})。當前價格 {last_close:.2f} 處於下降通道，需警惕資金流出 ({cmf_status})。"
        risk_level = "中高"
    elif rsi_status == "超買" and stoch_status == "超買" and total_score < 1.5:
        action = "🟡 潛在賣出 (Potential Sell)"
        reason = f"RSI ({df['RSI'].iloc[-1]:.2f}) 和 Stochastics ({df['Stoch_%K'].iloc[-1]:.2f}) 同時進入超買區，顯示短期回調風險增加。MACD 雖為 {macd_status}，但若多週期確認不明確，建議觀望或考慮部分獲利了結。"
        risk_level = "高"
    elif rsi_status == "超賣" and stoch_status == "超賣" and total_score > -1.5:
        action = "🟢 潛在買入 (Potential Buy)"
        reason = f"RSI ({df['RSI'].iloc[-1]:.2f}) 和 Stochastics ({df['Stoch_%K'].iloc[-1]:.2f}) 同時進入超賣區，顯示短期反彈機會。MACD 雖為 {macd_status}，但若多週期確認不明確，建議輕倉介入或等待更明確的底部訊號。"
        risk_level = "高"
    else:
        action = "⚪ 中性觀望 (Neutral)"
        reason = f"指標混雜或處於盤整區間。MACD 為 {macd_status}，RSI 位於中性區 ({rsi_status})。ADX ({adx_status}) 顯示趨勢可能不夠強勁。建議等待明確的多週期確認訊號或關鍵支撐壓力位突破 ({bb_breakout if bb_breakout else '無突破'})。"
        risk_level = "中"

    # --- 動態止盈止損建議 (基於斐波那契) ---
    fib_res = df['FIB_R3'].iloc[-1] # 最強阻力
    fib_sup = df['FIB_S3'].iloc[-1] # 最強支撐
    
    stop_loss = df['FIB_S2'].iloc[-1] if total_score > 0 else df['FIB_R2'].iloc[-1]
    take_profit = df['FIB_R2'].iloc[-1] if total_score > 0 else df['FIB_S2'].iloc[-1]

    
    # 格式化輸出
    analysis_text = f"""
    <div style='border: 2px solid #3498db; border-radius: 8px; padding: 15px; margin-top: 15px; background-color: #f0f8ff;'>
        <h4 style='color: #2c3e50;'>✨ {symbol_name} AI 專家總結與行動建議</h4>
        <p><strong>最新收盤價：</strong> <span style='color: #1abc9c; font-weight: bold;'>{last_close:.2f}</span></p>
        <p><strong>建議行動：</strong> {action}</p>
        <p><strong>主要考量：</strong> {reason}</p>
        <p><strong>綜合風險評級：</strong> {risk_level}</p>
        <hr style='border-top: 1px dashed #ccc;'>
        <h5 style='color: #2c3e50;'>📌 斐波那契動態止盈止損 (基於過去 {df.shape[0]} 根K線)</h5>
        <p>👉 **止盈目標 (Take Profit):** 約 <span style='color: #28a745; font-weight: bold;'>{take_profit:.2f}</span> (接近 FIB R2/S2)</p>
        <p>🛑 **止損目標 (Stop Loss):** 約 <span style='color: #dc3545; font-weight: bold;'>{stop_loss:.2f}</span> (接近 FIB R2/S2)</p>
        <p>🛡️ **關鍵支撐 (Support):** FIB S3 約 {fib_sup:.2f}</p>
        <p>⚔️ **關鍵阻力 (Resistance):** FIB R3 約 {fib_res:.2f}</p>
    </div>
    """
    
    return analysis_text, action


# ==============================================================================
# 3. Streamlit UI 介面與交互邏輯
# ==============================================================================

# 初始化 Session State
if 'last_search_symbol' not in st.session_state:
    st.session_state['last_search_symbol'] = "2330.TW"
if 'data_ready' not in st.session_state:
    st.session_state['data_ready'] = False
if 'sidebar_search_input' not in st.session_state:
    st.session_state['sidebar_search_input'] = ""


# --- 左側欄 (Sidebar) 配置 ---
st.sidebar.title("🛠️ 分析參數設定")

# A. 資產類別與標的選擇
asset_class = st.sidebar.radio(
    "選擇資產類別", 
    ["美股", "台股", "加密貨幣"], 
    index=1
)

# 根據資產類別過濾標的
if asset_class == "美股":
    filtered_symbols = {k: v for k, v in FULL_SYMBOLS_MAP.items() if k not in ["2330.TW", "2454.TW", "BTC-USD", "ETH-USD"]}
elif asset_class == "台股":
    filtered_symbols = {k: v for k, v in FULL_SYMBOLS_MAP.items() if ".TW" in k}
else:
    filtered_symbols = {k: v for k, v in FULL_SYMBOLS_MAP.items() if "-USD" in k}

symbol_list = list(filtered_symbols.keys())
name_list = [f"{v['name']} ({k})" for k, v in filtered_symbols.items()]
combined_list = list(zip(symbol_list, name_list))

# 下拉選單選擇
selected_name = st.sidebar.selectbox(
    "快速選擇熱門標的",
    name_list,
    index=0
)
# 獲取實際代碼
current_symbol = symbol_list[name_list.index(selected_name)]

# 允許自訂輸入 (使用者可輸入任何代碼)
st.session_state['sidebar_search_input'] = st.sidebar.text_input(
    "或輸入自訂代碼 (e.g., AAPL, 0050.TW)",
    value=current_symbol,
    key='symbol_input'
)

# 最終確認要使用的代碼
if st.session_state.symbol_input:
    final_symbol = st.session_state.symbol_input.upper()
else:
    final_symbol = current_symbol
    
symbol_display_name = FULL_SYMBOLS_MAP.get(final_symbol, {}).get("name", final_symbol)


# B. 週期與回測設定
st.sidebar.markdown("---")
st.sidebar.subheader("⏳ 週期與回測")

timeframe_option = st.sidebar.selectbox(
    "📊 K線週期 (Interval)",
    list(PERIOD_MAP.keys()),
    index=2 # 預設為 '1 日'
)

# --- 💡 優化後的掃描靈敏度/回溯K線數輸入 ---
st.sidebar.markdown("---")
st.sidebar.subheader("🔭 數據回溯範圍 (分析深度)")

# 1. 預設模式選擇，讓使用者選擇情境而非數字
preset_key = st.sidebar.selectbox(
    "🧠 分析深度預設模式",
    ["自動適應 (推薦)", "短期交易 (約 60 根K線)", "中期趨勢 (約 120 根K線)", "長期投資 (約 250 根K線)", "自訂K線數"],
    index=0,
    key='lookback_preset'
)

# 2. 處理預設與自訂邏輯
default_candle_count = 250 # 安全的預設值

if preset_key == "自動適應 (推薦)":
    # 根據選定的K線週期，設置一個合理的預設K線數
    if timeframe_option in ["30 分", "4 小時"]:
        lookback_candles = 100
    elif timeframe_option == "1 日":
        lookback_candles = 150
    elif timeframe_option == "1 週":
        lookback_candles = 250
    else:
        lookback_candles = default_candle_count
    
    st.sidebar.info(f"系統將根據週期自動選擇約 **{lookback_candles} 根** K線進行分析。")

elif preset_key == "短期交易 (約 60 根K線)":
    lookback_candles = 60
elif preset_key == "中期趨勢 (約 120 根K線)":
    lookback_candles = 120
elif preset_key == "長期投資 (約 250 根K線)":
    lookback_candles = 250
elif preset_key == "自訂K線數":
    # 顯示原始輸入框，並提供詳細的幫助資訊
    lookback_candles = st.sidebar.number_input(
        "📐 **自訂回溯K線數 (Advanced)**",
        min_value=10,
        max_value=1000,
        value=default_candle_count,
        step=10,
        key='lookback_custom'
    )
    
    st.sidebar.help("""
        **回溯K線數** (或分析深度) 決定了AI模型在進行計算時要看多少根歷史K線。
        
        - **數值越大**：分析結果越穩定，濾除雜訊效果越好，但對近期波動的反應會更慢。適合長期策略。
        - **數值越小**：分析結果越靈敏，對最新價格變化的反應越快，但可能包含較多雜訊或錯誤訊號。適合短期策略。
        
        **建議:** 除非您是專業人士，否則建議使用上方的「自動適應」或「預設模式」。一般情況下，使用 **100 到 250** 根K線是一個安全的範圍。
    """)

# 確保 lookback_candles 為整數
lookback_candles = int(lookback_candles)
# --- 💡 優化後的輸入結束 ---


# C. 執行分析按鈕
st.sidebar.markdown("---")
analyze_button = st.sidebar.button("📊 執行AI分析", type="primary")


# --- 主體內容 (Main Content) ---

# 頂部儀表板標題
st.markdown(f"""
    <div style="text-align: center; background-color: #2c3e50; padding: 10px; border-radius: 10px; color: white;">
        <h1 style="margin: 0;">📈 {symbol_display_name} ({final_symbol}) AI 趨勢分析儀表板</h1>
        <p style="margin: 0; font-size: 16px;">V12.3 - {timeframe_option}週期分析，回溯K線數: {lookback_candles}</p>
    </div>
""", unsafe_allow_html=True)


if analyze_button or st.session_state.data_ready:
    
    with st.spinner(f"正在為 {final_symbol} 執行 {timeframe_option} 週期AI分析..."):
        
        # 1. 獲取主要週期數據
        period, interval = PERIOD_MAP[timeframe_option]
        df, error = get_historical_data(final_symbol, period, interval)
        
        if error:
            st.error(error)
            st.session_state.data_ready = False
        else:
            # 2. 計算技術指標並裁切數據 (使用 lookback_candles)
            df = calculate_indicators(df, lookback_candles)
            
            if df.empty:
                 st.error(f"錯誤：在計算指標後，剩餘的有效數據不足。請嘗試減少回溯K線數或選擇更長的週期。")
                 st.session_state.data_ready = False
            else:
                st.session_state.data_ready = True
                
                # 3. 執行多週期趨勢確認 (獲取日線和週線數據)
                df_daily, _ = get_historical_data(final_symbol, "5y", "1d")
                df_weekly, _ = get_historical_data(final_symbol, "max", "1wk")
                
                # 計算多週期指標（僅需 MACD_Hist 和 ADX_9）
                if not df_daily.empty:
                    df_daily = calculate_indicators(df_daily, 250) # Use standard lookback for daily/weekly
                if not df_weekly.empty:
                     df_weekly = calculate_indicators(df_weekly, 250)
                
                multi_trend_summary, total_score = get_multi_timeframe_trend(df_daily, df_weekly)

                # 4. 生成 AI 分析總結
                ai_analysis_text, action_category = generate_ai_analysis(df, symbol_display_name, total_score)
                
                # --- 儀表板顯示 ---
                
                col1, col2 = st.columns([1, 2])
                
                with col1:
                    st.markdown(ai_analysis_text, unsafe_allow_html=True)
                    
                    st.markdown("---")
                    st.subheader("🌐 多週期趨勢確認")
                    st.markdown(multi_trend_summary, unsafe_allow_html=True)
                    
                    st.markdown("---")
                    st.subheader("數據概覽")
                    st.dataframe(df.tail(5)[['Open', 'High', 'Low', 'Close', 'Volume', 'RSI', 'MACD_Hist']], use_container_width=True)

                with col2:
                    st.subheader("🕯️ K線與斐波那契/布林帶")
                    
                    # 創建 K 線圖與指標子圖
                    fig = make_subplots(rows=4, cols=1, 
                                        shared_xaxes=True, 
                                        vertical_spacing=0.05,
                                        row_heights=[0.5, 0.15, 0.15, 0.2],
                                        subplot_titles=(
                                            f'{symbol_display_name} K線圖 & 趨勢線', 
                                            'MACD (9, 16, 5)', 
                                            'ADX & CMF',
                                            'RSI & Stochastics'
                                        ))

                    # Row 1: K線圖 (Candlestick)
                    fig.add_trace(go.Candlestick(x=df.index,
                                                open=df['Open'],
                                                high=df['High'],
                                                low=df['Low'],
                                                close=df['Close'],
                                                name='K線'), row=1, col=1)
                    
                    # Row 1: 斐波那契支撐與阻力線
                    fig.add_trace(go.Scatter(x=df.index, y=df['FIB_R3'], name='FIB R3 (阻力)', line=dict(color='red', dash='dot')), row=1, col=1)
                    fig.add_trace(go.Scatter(x=df.index, y=df['FIB_S3'], name='FIB S3 (支撐)', line=dict(color='green', dash='dot')), row=1, col=1)
                    
                    # Row 1: 布林帶
                    fig.add_trace(go.Scatter(x=df.index, y=df['BB_High'], name='BB High', line=dict(color='orange', width=1)), row=1, col=1)
                    fig.add_trace(go.Scatter(x=df.index, y=df['BB_Low'], name='BB Low', line=dict(color='purple', width=1)), row=1, col=1)
                    fig.add_trace(go.Scatter(x=df.index, y=df['BB_Mid'], name='BB Mid', line=dict(color='grey', width=1, dash='dash')), row=1, col=1)

                    # Row 2: MACD
                    fig.add_trace(go.Bar(x=df.index, y=df['MACD_Hist'], name='MACD Hist', marker_color=np.where(df['MACD_Hist'] > 0, '#28a745', '#dc3545')), row=2, col=1)
                    fig.add_trace(go.Scatter(x=df.index, y=df['MACD_Line'], name='MACD Line', line=dict(color='#007bff')), row=2, col=1)
                    fig.add_trace(go.Scatter(x=df.index, y=df['MACD_Signal'], name='Signal Line', line=dict(color='#ffc107')), row=2, col=1)
                    fig.add_hline(y=0, line_width=1, line_dash="dash", line_color="grey", row=2, col=1)

                    # Row 3: ADX & CMF
                    fig.add_trace(go.Scatter(x=df.index, y=df['ADX_9'], name='ADX', line=dict(color='brown')), row=3, col=1)
                    fig.add_hline(y=25, line_width=1, line_dash="dash", line_color="grey", row=3, col=1) # 25為趨勢強弱分界線
                    fig.add_trace(go.Bar(x=df.index, y=df['CMF'], name='CMF', marker_color=np.where(df['CMF'] > 0, '#28a745', '#dc3545')), row=3, col=1)
                    fig.add_hline(y=0, line_width=1, line_dash="dash", line_color="grey", row=3, col=1)

                    # Row 4: RSI & Stochastics
                    fig.add_trace(go.Scatter(x=df.index, y=df['Stoch_%K'], name='Stoch %K', line=dict(color='cyan')), row=4, col=1)
                    fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], name='RSI', line=dict(color='blue')), row=4, col=1)
                    fig.add_hrect(y0=80, y1=100, line_width=0, fillcolor="red", opacity=0.2, row=4, col=1); # 超買區
                    fig.add_hrect(y0=0, y1=20, line_width=0, fillcolor="green", opacity=0.2, row=4, col=1) # 超賣區
                    fig.add_hline(y=50, line_width=1, line_dash="dash", line_color="grey", row=4, col=1) # 中線

                    # 更新佈局
                    fig.update_layout(
                        height=1000, 
                        xaxis_rangeslider_visible=False,
                        template="plotly_dark", # 使用暗色主題
                        title_x=0.5,
                        hovermode="x unified"
                    )
                    
                    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(255,255,255,0.1)')
                    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(255,255,255,0.1)')

                    st.plotly_chart(fig, use_container_width=True)

                st.success(f"✅ {symbol_display_name} 的AI分析已完成！")

else:
    # 初始歡迎畫面
    st.markdown("""
        <div style="padding: 20px; text-align: center; background-color: #ecf0f1; border-radius: 10px; color: #34495e;">
            <h2>歡迎使用 AI 趨勢分析儀表板 V12.3</h2>
            <p>本工具集成了多週期確認、斐波那契動態模型及多項核心技術指標。</p>
            <p>請在左側欄設定您的 <span style='color: #2c3e50; font-weight: bold;'>標的</span>、<span style='color: #2c3e50; font-weight: bold;'>K線週期</span> 及 <span style='color: #2c3e50; font-weight: bold;'>分析深度</span>，然後點擊 <span style='color: #3498db; font-weight: bold;'>『📊 執行AI分析』</span> 按鈕開始。</p>
        </div>
    """, unsafe_allow_html=True)
    
    st.markdown("---")
    
    st.subheader("📝 使用步驟：")
    st.markdown("1. **選擇資產類別**：在左側欄選擇 `美股`、`台股` 或 `加密貨幣`。")
    st.markdown("2. **選擇標的**：使用下拉選單快速選擇熱門標的，或直接在輸入框中鍵入代碼或名稱。")
    st.markdown("3. **選擇週期**：決定分析的長度（例如：`30 分`、`4 小時`、`1 日`、`1 周`）。")
    st.markdown("4. **調整回溯範圍**：選擇 <span style='color: #3498db; font-weight: bold;'>『自動適應』</span> 或根據您的交易目標（短期/中期/長期）選擇預設模式。")
    st.markdown(f"5. **執行分析**：點擊 <span style='color: #3498db; font-weight: bold;'>『📊 執行AI分析』**</span>，AI將融合多維度指標提供交易策略。", unsafe_allow_html=True)
    
    st.markdown("---")


# ==============================================================================
# 4. 運行主邏輯
# ==============================================================================
# 不需要 __main__ 區塊，Streamlit 會自動運行
# if __name__ == '__main__':
#     pass
