...
專家增強最終版實作 (V13.0 - Price Action & Volume Engine)
...
核心功能：
- [頂級策略] 植入「價格行為分析模組」，自動識別支撐與阻力
- [頂級策略] 植入「量價關係分析模組」，根據四條黃金法則判斷趨勢健康度
- [AI 升級] AI 專家總結將融合價格行為與量價分析，提供更深度的戰術解讀
...
st.set_page_config(page_title="🚀 AI 趨勢分析專家系統", page_icon="📈", layout="wide")
...
<style>
    ...
    .positive { color: #dc3545; } .negative { color: #28a745; } .neutral { color: #ffc107; }
...
FULL_SYMBOLS_MAP = {
    "TSLA": {"name": "特斯拉", "keywords": ["特斯拉", "TSLA"]}, "NVDA": {"name": "輝達", "keywords": ["輝達", "NVDA"]},
    "AAPL": {"name": "蘋果", "keywords": ["蘋果", "AAPL"]}, "GOOGL": {"name": "谷歌/Alphabet", "keywords": ["谷歌", "GOOGL"]},
    "MSFT": {"name": "微軟", "keywords": ["微軟", "MSFT"]}, "SPY": {"name": "SPDR 標普500 ETF", "keywords": ["SPY"]},
    "^GSPC": {"name": "S&P 500 指數", "keywords": ["標普", "S&P500"]}, "2330.TW": {"name": "台積電", "keywords": ["台積電", "2330"]},
    "2317.TW": {"name": "鴻海", "keywords": ["鴻海", "2317"]}, "2454.TW": {"name": "聯發科", "keywords": ["聯發科", "2454"]},
    "0050.TW": {"name": "元大台灣50", "keywords": ["台灣50", "0050"]}, "^TWII": {"name": "台股指數", "keywords": ["台股指數", "加權"]},
    "BTC-USD": {"name": "比特幣", "keywords": ["比特幣", "BTC"]}, "ETH-USD": {"name": "以太坊", "keywords": ["以太坊", "ETH"]},
    "BNB-USD": {"name": "幣安幣", "keywords": ["幣安幣", "BNB"]}, "SOL-USD": {"name": "Solana", "keywords": ["Solana", "SOL"]},
    "DOGE-USD": {"name": "狗狗幣", "keywords": ["狗狗幣", "DOGE"]}, "ADA-USD": {"name": "艾達幣", "keywords": ["艾達幣", "ADA"]},
    "XRP-USD": {"name": "瑞波幣", "keywords": ["瑞波幣", "XRP"]}, "LINK-USD": {"name": "Chainlink", "keywords": ["Chainlink", "LINK"]},
    "ASTR-USD": {"name": "Astar", "keywords": ["Astar", "ASTR"]}, "WIF-USD": {"name": "dogwifhat", "keywords": ["WIF"]},
    "SHIB-USD": {"name": "柴犬幣", "keywords": ["柴犬幣", "SHIB"]}, "POPCAT-USD": {"name": "Popcat", "keywords": ["Popcat", "POPCAT"]},
}
...
ANALYSIS_HORIZONS = {"短線": {'5 分': ('7d', '5m'), '15 分': ('30d', '15m'), '1 小時': ('60d', '60m')}, "中線": {'15 分': ('30d', '15m'), '1 小時': ('60d', '60m'), '4 小時': ('1y', '4h')}, "長線": {'1 小時': ('60d', '60m'), '4 小時': ('1y', '4h'), '日線': ('5y', '1d')}}
...
def calculate_indicators(df):
    ...
    df['MACD_Hist'] = ta.trend.MACD(df['Close'], 9, 16, 5).macd_diff()
...
def analyze_price_action(df):
    peaks, _ = find_peaks(df['High'], distance=5, prominence=df['ATR_9'].mean()/2)
    troughs, _ = find_peaks(-df['Low'], distance=5, prominence=df['ATR_9'].mean()/2)
    
    swing_highs = df['High'].iloc[peaks]
    swing_lows = df['Low'].iloc[troughs]
    ...
    if last_high > prev_high and last_low > prev_low: return "<span class='positive'>上升趨勢 (高點與低點抬高)</span>"
    if last_high < prev_high and last_low < prev_low: return "<span class='negative'>下降趨勢 (高點與低點降低)</span>"
    return "<span class='neutral'>區間盤整</span>"
...
def analyze_volume_price(df):
    ...
    vol_up = latest['Volume'] > vol_avg

    if price_up and vol_up: return "<span class='positive'>價漲量增 (健康趨勢)</span>"
    if price_up and not vol_up: return "<span class='neutral'>價漲量縮 (趨勢警訊)</span>"
    if not price_up and vol_up: return "<span class='negative'>價跌量增 (恐慌拋售)</span>"
    if not price_up and not vol_up: return "<span class='neutral'>價跌量縮 (動能衰竭)</span>"
    return "N/A"
...
def generate_ai_fusion_signal(df, funda_scores, inst_hold_pct):
    ...
    # [專家升級] 新增價格行為與量價分析
    opinions['價格結構分析'] = analyze_price_action(df)
    opinions['量價關係分析'] = analyze_volume_price(df)
...
def generate_expert_summary(signal, risk, df, name, mtf_summary=None):
    ...
    summary = f"📈 「AI 觀點：目前 「{name}」 的價格結構為 {opinions['價格結構分析']}，量價關係呈現 {opinions['量價關係分析']}。結合指標判斷，市場「{condition}」，綜合建議為 「{colored_action}」。"
    if risk.get('is_trade_valid', True):
        summary += f" 基於您的 R 倍數設定，建議部位規模為 **{risk.get('position_size', 0):.4f}**，止損 (SL) 約在 「{risk['sl']:.2f}」，止盈 (TP) 約在 「{risk['tp']:.2f}」。」"
...
def run_multi_timeframe_analysis(symbol, name, strategy, total_capital, risk_percentage):
    ...
    def check_alignment(df, direction):
        latest = df.iloc[-1]
        vol_ok = latest['Volume'] > df['Volume'].rolling(20).mean().iloc[-1]
        if direction == "Up": return latest['MACD_Hist'] > 0 and latest['RSI_9'] > 50 and vol_ok
        elif direction == "Down": return latest['MACD_Hist'] < 0 and latest['RSI_9'] < 50 and vol_ok
        return False
    ...
    if check_alignment(df_long, trend_long) and check_alignment(df_mid, trend_long) and check_alignment(df_short, trend_long):
        if trend_long == "Up":
            final_action = "買進 (Buy)"
            summary = f"📈 「AI 觀點：在 **{tf_long}/{tf_mid}/{tf_short}** 的週期框架下，偵測到高度一致的**多頭趨勢共振**。所有週期的 MACD、RSI 與成交量均支持主要趨勢向上。綜合建議為 「<span class='positive'>{final_action}</span>」。」"
        else:
            final_action = "賣出 (Sell)"; summary = f"📉 「AI 觀點：在 **{tf_long}/{tf_mid}/{tf_short}** 的週期框架下，偵測到高度一致的**空頭趨勢共振**。所有週期的 MACD、RSI 與成交量均支持主要趨勢向下。綜合建議為 「<span class='negative'>{final_action}</span>」。」"
