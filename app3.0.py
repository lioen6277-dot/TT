...
專家增強最終版實作 (V7.4 - RSI/MACD Weighted)
...
def generate_ai_fusion_signal(df, funda_scores, inst_hold_pct):
    ...
    action = "觀望 (Neutral)"
    # 調整閾值以適應新的分數範圍
    if total_score >= 5.0: action = "買進 (Buy)"
    elif total_score >= 1.5: action = "中性偏買 (Hold/Buy)"
    elif total_score <= -5.0: action = "賣出 (Sell/Short)"
    elif total_score <= -1.5: action = "中性偏賣 (Hold/Sell)"
    
    confidence = min(100, int(50 + (total_score / 12) * 50))
...
def calculate_consensus_risk_management(df, action):
    ...
    if is_buy_signal:
        ...
        # 做空邏輯: SL > Entry, TP < Entry
        valid_sl = [s for s in stops['tp'] if s > entry] # 注意: 做空的止損是壓力位
        valid_tp = [t for t in stops['sl'] if t < entry] # 注意: 做空的止盈是支撐位

        # 共識 SL: 取最低的3個SL取平均 (最接近的壓力)
        consensus_sl = np.mean(sorted(valid_sl)[:3]) if len(valid_sl) >= 3 else (np.mean(valid_sl) if valid_sl else entry + 2 * atr)
        # 共識 TP: 取最高的3個TP取平均 (最接近的支撐)
