...
專家增強最終版實作 (V12.0 - Professional Resonance Engine)
...
核心功能：
- [頂級策略] 引入「三週期共振確認」分析引擎，實現最嚴格的信號過濾
...
版本：12.0.0 (Professional Resonance)
"""
...
def calculate_indicators(df):
    ...
    df['MACD_Hist'] = ta.trend.MACD(df['Close'], window_fast=9, window_slow=16, window_sign=5).macd_diff() # [策略升級]
...
def run_multi_timeframe_analysis(symbol, name, strategy):
    ...
    def check_alignment(df, direction):
        latest = df.iloc[-1]
        if direction == "Up":
            return latest['MACD_Hist'] > 0 and latest['RSI_9'] > 50
        elif direction == "Down":
            return latest['MACD_Hist'] < 0 and latest['RSI_9'] < 50
        return False
