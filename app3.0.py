...
專家增強最終版實作 (V7.9 - Expert Chart Layout)
...
核心功能：
- [專家圖表] 重新設計技術圖表，依照 AI 評分權重進行分區、優先顯示
- [穩定性強化] 提高數據驗證門檻，確保數據長度足以計算核心指標 (EMA 50)
- [專家升級] 引入動態適應功能，在數據不足時自動降級分析模型
- [穩定性修復] 增加 SuperTrend 指標計算的 failsafe
- [專家功能] 新增 AI 專家總結模組
- [專家功能] 整合市場狀態過濾機制
- [策略升級] 調整總量化評分系統，主要權重賦予RSI與MACD
- [重大升級] 引入多策略共識止盈止損模組
- ... (及所有早期功能)

開發者：程式碼專家 (Generated by Gemini)
版本：7.9.0 (Expert Chart)
"""
...
CATEGORY_MAP = {
    "美股": [c for c in FULL_SYMBOLS_MAP.keys() if not (c.endswith((".TW", "-USD")) or c.startswith("^TWII"))],
...
PERIOD_MAP = {'30 分': ('60d', '30m'), '4 小時': ('1y', '60m'), '1 日': ('5y', '1d'), '1 週': ('max', '1wk')}
...
def calculate_indicators(df):
    ...
    df['MACD_Hist'] = ta.trend.MACD(df['Close'], 8, 17, 9).macd_diff()
    ...
    try:
        ...
        if supertrend is not None and 'SUPERT_10_3.0' in supertrend.columns:
            df['SuperTrend'], df['SuperTrend_Direction'] = supertrend['SUPERT_10_3.0'], supertrend['SUPERTd_10_3.0']
...
def generate_ai_fusion_signal(df, funda_scores, inst_hold_pct):
    ...
    if ema_10 > ema_50 and prev['EMA_10'] <= prev['EMA_50']: ma_score, opinions['趨勢分析 (MA)'] = 3.5, "<span class='positive'>**🚀 黃金交叉 (10/50)**</span>"
    elif ema_10 < ema_50 and prev['EMA_10'] >= prev['EMA_50']: ma_score, opinions['趨勢分析 (MA)'] = -3.5, "<span class='negative'>**💀 死亡交叉 (10/50)**</span>"
    elif pd.notna(ema_200):
        if ema_10 > ema_50 > ema_200: ma_score, opinions['趨勢分析 (MA)'] = 2.0, "<span class='positive'>強勢多頭排列: **10 > 50 > 200**</span>"
        elif ema_10 < ema_50 < ema_200: ma_score, opinions['趨勢分析 (MA)'] = -2.0, "<span class='negative'>強勢空頭排列: **10 < 50 < 200**</span>"
        else: ma_score, opinions['趨勢分析 (MA)'] = 0, "<span class='neutral'>中性盤整</span>"
    else:
        ma_score = 1.0 if ema_10 > ema_50 else -1.0
        opinions['趨勢分析 (MA)'] = f"<span class='{'positive' if ma_score > 0 else 'negative'}'>短期{'多' if ma_score > 0 else '空'}頭 (10 {' > ' if ma_score > 0 else ' < '} 50)</span> <span class='neutral'>(長線數據不足)</span>"
    rsi = latest['RSI_9']
    if rsi < 30: momentum_score, opinions['動能分析 (RSI 9)'] = 2.0, f"<span class='positive'>強化: RSI {rsi:.1f} < 30, 超賣</span>"
    elif rsi > 70: momentum_score, opinions['動能分析 (RSI 9)'] = -2.0, f"<span class='negative'>警告: RSI {rsi:.1f} > 70, 超買</span>"
    ...
    adx = latest['ADX_9']
    if adx > 25: strength_score *= 1.5; opinions['趨勢強度 (ADX 9)'] = f"<span class='positive'>確認強趨勢: ADX {adx:.1f} > 25</span>"
    else: opinions['趨勢強度 (ADX 9)'] = f"<span class='neutral'>盤整市: ADX {adx:.1f} < 25</span>"
...
def get_individual_stops(df):
    ...
    if 'SuperTrend' in df.columns and pd.notna(latest['SuperTrend']):
        (stops['sl'] if latest['SuperTrend_Direction'] == 1 else stops['tp']).append(latest['SuperTrend'])
    return stops
...
def generate_expert_summary(signal, risk, df, name):
    ...
    if "買" in action: summary = f"📈 **AI 觀點：** 目前 **{name}** 技術面偏多，由 {driver} 帶動。市場**{condition}**，建議 **<span class='positive'>{action}</span>**。共識支撐 (SL) 約在 **{risk['sl']:.2f}**，壓力 (TP) 約在 **{risk['tp']:.2f}**。"
    elif "賣" in action: summary = f"📉 **AI 觀點：** **{name}** 出現偏空警訊，由 {driver} 驅動。市場**{condition}**，建議 **<span class='negative'>{action}</span>**。共識壓力 (SL) 約在 **{risk['sl']:.2f}**，支撐 (TP) 約在 **{risk['tp']:.2f}**。"
    else: summary = f"📊 **AI 觀點：** **{name}** 處於**中性觀望**。市場**{condition}**，未形成一致信號。關鍵支撐與壓力在 **{risk['sl']:.2f}** 和 **{risk['tp']:.2f}**。"
...
def get_technical_data_df(df):
    ...
    if 'EMA_200' in df.columns and pd.notna(latest['EMA_200']):
        ...
        conclusion = "<span class='positive'>**強多頭：MA 多頭排列**</span>" if ema_10 > ema_50 > ema_200 else "<span class='neutral'>中性：MA 糾結</span>"
        data.append(['價格 vs. EMA 10/50/200', f"{latest['Close']:,.2f}", conclusion])
    else:
        ema_10, ema_50 = latest['EMA_10'], latest['EMA_50']
        conclusion = f"<span class='{'positive' if ema_10 > ema_50 else 'negative'}'>短期{'多' if ema_10 > ema_50 else '空'}頭</span>"
        data.append(['價格 vs. EMA 10/50', f"{latest['Close']:,.2f}", conclusion])
    rsi = latest['RSI_9']
    data.append(['RSI (9) 動能', f"{rsi:.2f}", f"<span class='negative'>超買 ({rsi:.1f})</span>" if rsi > 70 else (f"<span class='positive'>超賣 ({rsi:.1f})</span>" if rsi < 30 else '中性')])
    adx = latest['ADX_9']
    data.append(['ADX (9) 趨勢強度', f"{adx:.2f}", f"<span class='positive'>強趨勢 ({adx:.1f})</span>" if adx > 25 else '盤整'])
    return pd.DataFrame(data, columns=['指標名稱', '最新值', '分析結論']).set_index('指標名稱')

# --- 3. Streamlit 使用者介面 (UI) ---
with st.sidebar:
    st.subheader("1. 選擇資產類別")
    selected_category = st.selectbox("", list(CATEGORY_HOT_OPTIONS.keys()), on_change=lambda: st.session_state.update(manual_input=""), key="sb_category", label_visibility="collapsed")
    st.subheader("2. 選擇或輸入標的")
    selected_asset = st.selectbox("", list(CATEGORY_HOT_OPTIONS[selected_category].keys()), on_change=lambda: st.session_state.update(manual_input=""), key="sb_asset", label_visibility="collapsed")
    ...
    st.markdown("---")
    st.subheader("3. 選擇週期")
    selected_period_label = st.selectbox("", list(PERIOD_MAP.keys()), index=2, label_visibility="collapsed")
    ...
    with st.spinner(f'AI 分析引擎啟動中，正在分析 {final_symbol_name}...'):
        ...
        if data is not None and len(data) > 55:
            ...
            if df.empty or len(df) < 2:
                st.error("錯誤：數據清洗後不足以進行分析，請嘗試更長的分析週期。")
            else:
                ...
                <div class="strategy-summary">💡 <b>策略總結:</b> 基於 {signal['action']} 信號，在 <b>~{risk['entry']:,.2f}</b> 範圍尋找機會 | ⚖️ <b>R:R:</b> {rr if isinstance(rr, str) else f'{rr:.2f}'} | ATR: {risk['atr']:.4f}</div>""", unsafe_allow_html=True)
                ...
                fig = make_subplots(
                    ...
                    subplot_titles=("價格與趨勢指標", "核心動能 (RSI / MACD)", "趨勢強度與資金流 (ADX / CMF)", "擺盪指標 (Stochastic %K)")
                )
                ...
                # Row 3: Trend Strength & Money Flow
                ...
                fig.update_layout(height=1000, xaxis_rangeslider_visible=False, template="plotly_dark", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
                fig.update_yaxes(title_text="價格", row=1, col=1)
                fig.update_yaxes(title_text="RSI/MACD", row=2, col=1)
                fig.update_yaxes(title_text="ADX/CMF", row=3, col=1)
                fig.update_yaxes(title_text="Stoch %K", row=4, col=1)
