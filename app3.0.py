...
專家增強最終版實作 (V12.0 - R-Multiple Strategy Engine)
...
核心功能：
- [頂級策略] 整合「R倍數風險管理交易策略」，實現動態部位規模計算
...
def calculate_r_multiple_strategy(df, action, total_capital, risk_percentage):
    ...
    if is_buy:
        ...
        take_profit_price = entry_price + 2 * (entry_price - stop_loss_price) # Default 2R
    else: # is_sell
        stop_loss_price = swing_high * 1.01
        take_profit_price = entry_price - 2 * (stop_loss_price - entry_price) # Default 2R
    ...
    return {
        ...
        'one_r_amount': one_r_amount,
        'r_value_per_unit': r_value_per_unit
    }
...
def generate_expert_summary(signal, risk, df, name, mtf_summary=None):
    ...
    if "買" in action: summary = f"📈 「AI 觀點：目前 「{name}」 技術面偏多，由 「{driver}」 帶動。市場「{condition}」，建議 「{colored_action}」。止損 (SL) 約在 「{risk['sl']:.2f}」，止盈 (TP) 約在 「{risk['tp']:.2f}」。」"
    elif "賣" in action: summary = f"📉 「AI 觀點：目前 「{name}」 技術面偏空，由 「{driver}」 帶動。市場「{condition}」，建議 「{colored_action}」。止損 (SL) 約在 「{risk['sl']:.2f}」，止盈 (TP) 約在 「{risk['tp']:.2f}」。」"
...
def run_multi_timeframe_analysis(symbol, name, strategy, total_capital, risk_percentage):
    ...
    if check_alignment(df_long, trend_long) and check_alignment(df_mid, trend_long) and check_alignment(df_short, trend_long):
        if trend_long == "Up":
            ...
            final_action = "賣出 (Sell)"
            summary = f"📉 「AI 觀點：在 **{tf_long}/{tf_mid}/{tf_short}** 的週期框架下，偵測到高度一致的**空頭趨勢共振**。所有週期的 MACD 與 RSI 指標均支持主要趨勢向下。綜合建議為 「<span class='negative'>{final_action}</span>」。」"
    else:
        summary = f"📊 「AI 觀點：在 {tf_long}/{tf_mid}/{tf_short} 的多週期框架下，未發現明顯的共振交易信號。主要趨勢 ({tf_long}) 為「{trend_long}」，但中短期指標未完全對齊。建議「<span class='neutral'>{final_action}</span>」，等待更清晰的結構。」"
        
    risk = calculate_r_multiple_strategy(df_long, final_action, total_capital, risk_percentage)
    summary += f" 基於您的 R 倍數設定，最大風險 (1R) 為 **{risk.get('one_r_amount', 'N/A'):.2f} USDT**。"
    ...
    if 'is_trade_valid' in risk and risk['is_trade_valid']:
        ...
        st.warning("根據 R 倍數風險管理原則 (盈虧比需 > 2)，目前未找到值得執行的交易機會。")
...
with st.sidebar:
    ...
    st.markdown(f"**單筆最大風險 (1R):** `${total_capital * (risk_percentage / 100):.2f}`")
    st.markdown("---")
    ...
    st.markdown("---"); st.subheader("7. 開始分析"); start_analysis = st.button("📊 執行AI分析", use_container_width=True)
...
if not start_analysis:
    ...
    st.markdown("1. **設定風險**: 在左側輸入您的總資金與單筆風險偏好。\n"
                "2. **選擇資產**: 透過類別或手動輸入。\n"
            ...
            if data is not None and len(data) > 55:
                ...
                if df.empty or len(df) < 2: st.error("錯誤：數據清洗後不足以進行分析，請嘗試更長的分析週期。")
                    ...
                    if risk['is_trade_valid']:
                        action_style = 'positive' if '買' in signal['action'] else 'negative'
                        st.subheader("🛡️ R倍數風險管理 & 部位規劃")
                        st.markdown(f"""<div class="metric-container">
                            ...
                            <div class="metric-card"><div class="metric-label">💵 建議入場價</div><div class="metric-value" style="color: #FA8072; font-weight: bold;">~{risk['entry']:,.4f}</div></div>
                            <div class="metric-card"><div class="metric-label">🛡️ 技術止損價 (SL)</div><div class="metric-value {'negative' if '買' in signal['action'] else 'positive'}">{risk['sl']:,.4f}</div></div>
                            <div class="metric-card"><div class="metric-label">🎯 主要止盈目標 (TP)</div><div class="metric-value {'positive' if '買' in signal['action'] else 'negative'}">{risk['tp']:,.4f}</div></div>
                        </div>
                        <div class="metric-container">
                            <div class="metric-card"><div class="metric-label">⚖️ 風險回報比 (R:R)</div><div class="metric-value {'positive' if risk['risk_reward_ratio'] >= 2 else 'neutral'}">{risk['risk_reward_ratio']:.2f}</div></div>
                            <div class="metric-card"><div class="metric-label">📉 單筆最大風險 (1R)</div><div class="metric-value negative">{risk['one_r_amount']:.2f} USDT</div></div>
