...
def calculate_fibonacci_risk_levels(df, action):
    ...
    lookback = 100 # Look back 100 candles for the swing
    ...
    if is_buy:
        # For long, TP are levels above entry, SL are levels below
        tp_options = [lvl for lvl in fibo_levels if lvl > entry]
        sl_options = [lvl for lvl in fibo_levels if lvl < entry]
        tp = min(tp_options) if tp_options else swing_high * 1.01 # Target nearest fib level or slightly above swing high
        sl = max(sl_options) if sl_options else swing_low * 0.99   # Stop below nearest fib level or swing low
    else: # is_sell
        tp_options = [lvl for lvl in fibo_levels if lvl < entry]
        sl_options = [lvl for lvl in fibo_levels if lvl > entry]
        tp = max(tp_options) if tp_options else swing_low * 0.99
        sl = min(sl_options) if sl_options else swing_high * 1.01

    return {'sl': sl, 'tp': tp, 'entry': entry, 'atr': df.iloc[-1]['ATR_9']}

def generate_expert_summary(signal, risk, name, mtf_summary=None):
    if mtf_summary: # If it's a multi-timeframe analysis
        return f"<div class='expert-summary'>{mtf_summary}</div>"
        
    action, opinions = signal['action'], signal['opinions']
    driver = f"RSI ({signal['opinions'].get('RSI_val', 0):.1f})" if "RSI" in opinions.get('動能分析 (RSI 9)', '') else "趨勢與動能指標"
    if "MACD" in opinions.get('趨勢強度 (MACD)', ''): driver = f"MACD 與 RSI ({signal['opinions'].get('RSI_val', 0):.1f})"
    condition = "趨勢明確" if "強趨勢" in opinions.get('趨勢強度 (ADX 9)', '') else "盤整"
...
def run_multi_timeframe_analysis(symbol, name):
    # Fetch data for all three timeframes
    df_4h = get_data(symbol, '1y', '4h')
    df_1h = get_data(symbol, '60d', '60m')
    df_15m = get_data(symbol, '30d', '15m')
    
    if df_4h is None or df_1h is None or df_15m is None or len(df_4h)<55 or len(df_1h)<55 or len(df_15m)<55:
        st.error("多週期分析所需數據不足，請確認標的或稍後再試。")
        return
        
    # Calculate indicators for each
    df_4h = calculate_indicators(df_4h.copy())
    df_1h = calculate_indicators(df_1h.copy())
    df_15m = calculate_indicators(df_15m.copy())

    # --- Hierarchical Analysis Logic ---
    # 1. Determine main trend from 4h chart
    trend_4h = "Up" if df_4h.iloc[-1]['EMA_50'] > df_4h.iloc[-1].get('EMA_200', 0) and df_4h.iloc[-1]['ADX_9'] > 20 else \
               "Down" if df_4h.iloc[-1]['EMA_50'] < df_4h.iloc[-1].get('EMA_200', 0) and df_4h.iloc[-1]['ADX_9'] > 20 else \
               "Sideways"

    # 2. Look for patterns in 1h chart
    death_cross_1h = df_1h.iloc[-1]['EMA_10'] < df_1h.iloc[-1]['EMA_50'] and df_1h.iloc[-2]['EMA_10'] >= df_1h.iloc[-2]['EMA_50']
    
    # 3. Look for entry trigger in 15m chart
    macd_turning_up_15m = df_15m.iloc[-1]['MACD_Hist'] > df_15m.iloc[-2]['MACD_Hist'] and df_15m.iloc[-1]['MACD_Hist'] < 0
    rsi_bottom_15m = df_15m.iloc[-1]['RSI_9'] < 30

    # Synthesize the final signal
    final_action = "觀望 (Neutral)"
    if trend_4h == "Up" and death_cross_1h and (macd_turning_up_15m or rsi_bottom_15m):
        final_action = "買進 (Buy)"
        summary = f"📈 「AI 觀點：基於 **4 小時**週期的主要**上漲趨勢**，AI 觀察到 **1 小時**週期出現一個潛在的**誘空死亡交叉**。同時，**15 分鐘**圖的動能指標（MACD/RSI）顯示**反轉跡象**，構成一個多頭共振的進場機會。綜合建議為 「<span class='positive'>{final_action}</span>」。」"
    else:
        summary = f"📊 「AI 觀點：目前在 4h/1h/15m 的多週期框架下，未發現明顯的共振交易信號。主要趨勢（4h）為「{trend_4h}」，但中短期訊號不一致。建議「<span class='neutral'>{final_action}</span>」，等待更清晰的結構。」"
        
    # Use 4h data for risk management
    risk = calculate_fibonacci_risk_levels(df_4h, final_action)
    ...
    # Display a simplified metrics view for MTF
    st.subheader("🛡️ 斐波那契交易策略與風險控制")
    action_style = 'positive' if '買' in final_action else ('negative' if '賣' in final_action else 'neutral')
    is_buy = "買" in final_action
    ...
    st.markdown(f"""<div class="metric-container">
        ...
        <div class="metric-card"><div class="metric-label">當前價格</div><div class="metric-value" style="color: #FA8072; font-weight: bold;">~{risk['entry']:,.2f}</div></div>
...
with st.sidebar:
    ...
    st.subheader("3. 選擇分析策略")
    analysis_strategy = st.selectbox("", ["單一週期分析", "多週期趨勢確認"], label_visibility="collapsed")
...
if not start_analysis:
    ...
    if analysis_strategy == "多週期趨勢確認":
        run_multi_timeframe_analysis(final_symbol, final_symbol_name)
    else: # Single Timeframe Analysis
