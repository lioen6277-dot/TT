...
專家增強最終版實作 (V5.7 - UI Overhaul & Content Expansion)
...
- [升級] 全局色彩風格重塑為水泥灰色調 (#7F8274)
- [升級] 為「建議入場價」套用指定的鮭魚色 (#FA8072)
- [升級] 統一全站色彩邏輯 (紅=多頭/漲, 綠=空頭/跌)
...
版本：5.7.0 (UI Overhaul & Content Expansion)
"""
...
# (UPDATED v5.7) 全局 CSS 樣式，實現水泥灰色調主題
st.markdown("""
<style>
    /* 全局水泥灰主題 */
    body, .stApp {
        background-color: #7F8274;
        color: #1c1c1c; /* 深色文字以確保可讀性 */
    }
    /* 側邊欄樣式 */
    [data-testid="stSidebar"] {
        background-color: #707365; /* 較深的灰色 */
        border-right: 1px solid #606355;
    }
    /* 主要內容區塊 */
    ...
    /* 標題樣式 */
    h1, h2, h3, .st-info, .st-emotion-cache-16idsys p {
        color: #1c1c1c !important; /* 強制設定標題與文字顏色 */
        font-weight: 600;
    }
    /* 自訂指標卡片容器 */
    .metric-container {
    ...
    .metric-card {
        background-color: #8A8D7E; /* 稍淺的灰色 */
        border-radius: 8px;
        padding: 15px;
        flex-grow: 1;
        border: 1px solid #707365;
    }
    .metric-label {
        font-size: 0.9em;
        color: #3d3d3d;
    ...
    .custom-table th, .custom-table td {
        padding: 12px 15px;
        text-align: left;
        border-bottom: 1px solid #707365;
    }
    .custom-table th {
        background-color: #8A8D7E;
        color: #3d3d3d;
        font-weight: 500;
    }
    .custom-table td {
    ...
    /* Plotly 圖表背景透明化 */
    .js-plotly-plot .plotly-layout, .js-plotly-plot .plotly .main-svg {
...
def get_technical_data_df(df):
    ...
    data.append(['MACD (8/17/9) 柱狀圖', f"{latest['MACD_Hist']:.2f}", "<span class='neutral'>動能盤整</span>"])

    adx = latest['ADX_9']
    if adx > 25: conclusion_adx = f"<span class='positive'>強趨勢 ({adx:.1f})</span>"
    else: conclusion_adx = "<span class='neutral'>盤整</span>"
    data.append(['ADX (9) 趨勢強度', f"{adx:.2f}", conclusion_adx])

    data.append(['ATR (9) 波動性', f"{latest['ATR_9']:.4f}", "<span class='neutral'>正常波動性</span>"])
    ...
    with st.spinner(f'AI 分析引擎啟動中...'):
        ...
        if data is not None and len(data) > 50:
            ...
            if df.empty:
                ...
                confidence_color_class = 'positive' if signal['total_score'] >= 0 else 'negative'
                ...
                <div class="metric-container">
                    ...
                    <div class="metric-card"><div class="metric-label">🛡️ 信心指數</div><div class="metric-value"><span class="{confidence_color_class}">{signal['confidence']}%</span></div></div>
                </div>
                """, unsafe_allow_html=True)
                ...
                if backtest and backtest.get("total_trades", 0) > 0:
                    ...
                    <div class="metric-container">
                        ...
                        <div class="metric-card"><div class="metric-label">📉 最大回撤 (MDD)</div><div class="metric-value">{abs(backtest['max_drawdown']):.2f}%</div></div>
                    ...
                    fig_bt.update_layout(title='SMA 20/EMA 50 交叉策略資金曲線', template=None, height=300, margin=dict(l=20, r=20, t=40, b=20),
                                         paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color='#1c1c1c')
                ...
                fig.update_yaxes(title_text="價格", row=1, col=1, secondary_y=False, gridcolor='rgba(128,128,128,0.2)')
                fig.update_yaxes(showticklabels=False, showgrid=False, zeroline=False, row=1, col=1, secondary_y=True)
                
                fig.update_yaxes(title_text="指標", row=2, col=1, secondary_y=False, gridcolor='rgba(128,128,128,0.2)')
