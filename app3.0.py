...
專家增強最終版實作 (V7.7 - Dynamic Adaptation Patch)
...
核心功能：
- [專家升級] 引入動態適應功能，在數據不足時自動降級分析模型，避免崩潰
- [穩定性修復] 增加 SuperTrend 指標計算的 failsafe，防止因數據不足導致的 KeyError 崩潰
- [專家功能] 新增 AI 專家總結模組，提供敘事型分析結論
- [專家功能] 整合市場狀態過濾機制，在盤整市中自動降低信號權重
- [策略升級] 調整總量化評分系統，主要權重賦予RSI與MACD指標
- [重大升級] 引入多策略共識止盈止損模組，取代單一ATR策略
...
def calculate_indicators(df):
    ...
    # [穩定性修復] SuperTrend Failsafe
    try:
        supertrend = df.ta.supertrend(length=10, multiplier=3)
        if supertrend is not None and not supertrend.empty and 'SUPERT_10_3.0' in supertrend.columns:
...
def generate_ai_fusion_signal(df, funda_scores, inst_hold_pct):
    ...
    # 檢查黃金/死亡交叉
    golden_cross = ema_10 > ema_50 and prev['EMA_10'] <= prev['EMA_50']
    death_cross = ema_10 < ema_50 and prev['EMA_10'] >= prev['EMA_50']

    if golden_cross:
        ma_score, opinions['趨勢分析 (MA)'] = 3.5, "<span class='positive'>**🚀 黃金交叉 (10/50)**</span>"
    elif death_cross:
        ma_score, opinions['趨勢分析 (MA)'] = -3.5, "<span class='negative'>**💀 死亡交叉 (10/50)**</span>"
    elif pd.notna(ema_200): # 如果有 EMA_200，則判斷三線排列
        if ema_10 > ema_50 and ema_50 > ema_200:
            ma_score, opinions['趨勢分析 (MA)'] = 2.0, "<span class='positive'>強勢多頭排列: **10 > 50 > 200**</span>"
        elif ema_10 < ema_50 and ema_50 < ema_200:
            ma_score, opinions['趨勢分析 (MA)'] = -2.0, "<span class='negative'>強勢空頭排列: **10 < 50 < 200**</span>"
        else:
            ma_score, opinions['趨勢分析 (MA)'] = 0, "<span class='neutral'>中性盤整</span>"
    else: # 如果沒有 EMA_200，則降級為雙線判斷
        if ema_10 > ema_50:
            ma_score, opinions['趨勢分析 (MA)'] = 1.0, "<span class='positive'>短期多頭 (10 > 50)</span>"
        else:
            ma_score, opinions['趨勢分析 (MA)'] = -1.0, "<span class='negative'>短期空頭 (10 < 50)</span>"
        opinions['趨勢分析 (MA)'] += " <span class='neutral'>(長線趨勢數據不足)</span>"
    ...
    adx_value = latest['ADX_9']
    if adx_value > 25:
        strength_score *= 1.5; opinions['趨勢強度 (ADX 9)'] = f"<span class='positive'>確認強趨勢: ADX {adx_value:.1f} > 25</span>"
    else:
        opinions['趨勢強度 (ADX 9)'] = f"<span class='neutral'>盤整市: ADX {adx_value:.1f} < 25</span>"
    ...
    total_score = (momentum_score * weights['momentum'] + strength_score * weights['strength'] + ma_score * weights['ma'] + fa_norm * weights['fa'])
    
    market_regime_filter = 1.0
    if adx_value < 20:
        market_regime_filter = 0.6
        opinions['市場狀態'] = f"<span class='neutral'>注意：市場處於低波動盤整期 (ADX < 20)，趨勢信號可能較弱。</span>"
    else:
        opinions['市場狀態'] = f"<span class='positive'>清晰的趨勢市場 (ADX > 20)。</span>"
    
    total_score *= market_regime_filter
...
def get_individual_stops(df):
    stops = {'tp': [], 'sl': []}
    latest = df.iloc[-1]
    stops['sl'].append(latest['Close'] - 2 * latest['ATR_9'])
    stops['tp'].append(latest['Close'] + 4 * latest['ATR_9'])
    stops['sl'].append(latest['BB_Low'])
    stops['tp'].append(latest['BB_High'])
    if 'SuperTrend' in df.columns and pd.notna(latest['SuperTrend']):
        if latest['SuperTrend_Direction'] == 1: stops['sl'].append(latest['SuperTrend'])
        else: stops['tp'].append(latest['SuperTrend'])
    recent_period = df.tail(20)
    stops['sl'].append(recent_period['Low'].min())
    stops['tp'].append(recent_period['High'].max())
    return stops

def calculate_consensus_risk_management(df, action):
    entry, atr = df.iloc[-1]['Close'], df.iloc[-1]['ATR_9']
    stops = get_individual_stops(df)
    is_buy_signal = "買" in action or "Buy" in action
    if is_buy_signal:
        valid_sl = [s for s in stops['sl'] if s < entry]
        valid_tp = [t for t in stops['tp'] if t > entry]
        consensus_sl = np.mean(sorted(valid_sl, reverse=True)[:3]) if len(valid_sl) >= 3 else (np.mean(valid_sl) if valid_sl else entry - 2 * atr)
        consensus_tp = np.mean(sorted(valid_tp)[:3]) if len(valid_tp) >= 3 else (np.mean(valid_tp) if valid_tp else entry + 4 * atr)
    else:
        valid_sl = [s for s in stops['tp'] if s > entry]
        valid_tp = [t for t in stops['sl'] if t < entry]
        consensus_sl = np.mean(sorted(valid_sl)[:3]) if len(valid_sl) >= 3 else (np.mean(valid_sl) if valid_sl else entry + 2 * atr)
        consensus_tp = np.mean(sorted(valid_tp, reverse=True)[:3]) if len(valid_tp) >= 3 else (np.mean(valid_tp) if valid_tp else entry - 4 * atr)
    return {'sl': consensus_sl, 'tp': consensus_tp, 'entry': entry, 'atr': atr}

# --- [專家建議] AI 專家總結 ---
def generate_expert_summary(signal, risk, df, symbol_name):
    action, opinions = signal['action'], signal['opinions']
    main_driver = "趨勢與動能指標"
    if "RSI" in opinions.get('動能分析 (RSI 9)', ''): main_driver = f"**RSI ({df.iloc[-1]['RSI_9']:.1f})**"
    if "MACD" in opinions.get('趨勢強度 (MACD)', ''): main_driver = f"**MACD** 與 **RSI ({df.iloc[-1]['RSI_9']:.1f})**"
    market_condition = "趨勢明確" if df.iloc[-1]['ADX_9'] > 20 else "盤整"
    if "買" in action:
        summary = f"📈 **AI 觀點：** 目前 **{symbol_name}** 的技術面呈現**偏多格局**。主要的驅動因子來自 {main_driver} 的正向信號。當前市場狀態為 **{market_condition}**，AI 系統給出的行動建議為 **<span class='positive'>{action}</span>**。"
        summary += f" 基於多策略共識模型，關鍵的支撐區域 (止損參考) 位於 **{risk['sl']:.2f}** 附近，而上方的潛在壓力 (止盈參考) 則在 **{risk['tp']:.2f}** 左右。"
    elif "賣" in action:
        summary = f"📉 **AI 觀點：** **{symbol_name}** 當前的技術結構出現**偏空警訊**。此信號主要由 {main_driver} 帶動。市場狀態處於 **{market_condition}**，系統綜合判斷後的行動建議為 **<span class='negative'>{action}</span>**。"
        summary += f" 根據共識模型，上方的壓力區 (止損參考) 位於 **{risk['sl']:.2f}**，而下方的支撐 (止盈參考) 可能在 **{risk['tp']:.2f}** 附近。"
    else:
        summary = f"📊 **AI 觀點：** **{symbol_name}** 目前處於**中性觀望**狀態。儘管部分指標各有強弱，但尚未形成一致的突破信號。市場狀態為 **{market_condition}**，建議保持耐心，等待更明確的趨勢出現。關鍵支撐與壓力分別位於 **{risk['sl']:.2f}** 和 **{risk['tp']:.2f}**。"
    return f"<div class='expert-summary'>{summary}</div>"

def run_backtest(df, initial_capital=100000):
    if df.empty or 'SMA_20' not in df.columns or 'EMA_50' not in df.columns: return {}
    df['signal'] = 0
    df.loc[(df['SMA_20'] > df['EMA_50']) & (df['SMA_20'].shift(1) <= df['EMA_50'].shift(1)), 'signal'] = 1
    df.loc[(df['SMA_20'] < df['EMA_50']) & (df['SMA_20'].shift(1) >= df['EMA_50'].shift(1)), 'signal'] = -1
    cash = initial_capital
    position_size, entry_price = 0, 0
    trades, equity_curve_values = [], []
    for i in range(len(df)):
        if df['signal'].iloc[i] == 1 and position_size == 0:
            position_size = cash / df['Close'].iloc[i]; entry_price = df['Close'].iloc[i]; cash = 0
        elif df['signal'].iloc[i] == -1 and position_size > 0:
            cash = position_size * df['Close'].iloc[i]; trades.append(cash - (position_size * entry_price)); position_size = 0
        equity_curve_values.append(cash + (position_size * df['Close'].iloc[i]))
    if position_size > 0: trades.append((position_size * df['Close'].iloc[-1]) - (position_size * entry_price))
    if not trades: return {"total_return": 0, "win_rate": 0, "max_drawdown": 0, "total_trades": 0, "equity_curve": pd.Series([initial_capital]*len(df), index=df.index)}
    ...
    daily_drawdown = equity_curve / roll_max - 1.0; max_drawdown = abs(daily_drawdown.min() * 100)
    return {"total_return": total_return, "win_rate": win_rate, "max_drawdown": max_drawdown, "total_trades": len(trades), "equity_curve": equity_curve}
...
def get_technical_data_df(df):
    ...
    # [動態適應] 根據是否有 EMA_200 調整顯示
    if 'EMA_200' in df.columns and pd.notna(latest['EMA_200']):
        ema_10, ema_50, ema_200 = latest['EMA_10'], latest['EMA_50'], latest['EMA_200']
        conclusion_ma = "<span class='positive'>**強多頭：MA 多頭排列**</span>" if ema_10 > ema_50 and ema_50 > ema_200 else "<span class='neutral'>中性：MA 糾結或趨勢發展中</span>"
        data.append(['價格 vs. EMA 10/50/200', f"{latest['Close']:,.2f}", conclusion_ma])
    else:
        ema_10, ema_50 = latest['EMA_10'], latest['EMA_50']
        conclusion_ma = "<span class='positive'>短期多頭</span>" if ema_10 > ema_50 else "<span class='negative'>短期空頭</span>"
        data.append(['價格 vs. EMA 10/50', f"{latest['Close']:,.2f}", conclusion_ma])
    ...
    adx = latest['ADX_9']
    conclusion_adx = f"<span class='positive'>強趨勢 ({adx:.1f})</span>" if adx > 25 else f"<span class='neutral'>盤整</span>"
...
with st.sidebar:
    ...
    selected_category = st.selectbox("資產類別", list(CATEGORY_HOT_OPTIONS.keys()), on_change=lambda: st.session_state.update(manual_input=""), key="sb_category", label_visibility="collapsed")
    st.subheader("2. 選擇或輸入標的")
    selected_asset = st.selectbox("熱門標的", list(CATEGORY_HOT_OPTIONS[selected_category].keys()), on_change=lambda: st.session_state.update(manual_input=""), key="sb_asset", label_visibility="collapsed")
...
if not start_analysis:
    ...
    st.markdown("1.  **選擇資產類別**: 在左側欄選擇 `美股`、`台股` 或 `加密貨幣`。\n"
                ...
                "4.  **執行分析**: 點擊 <span style='color: #FA8072; font-weight: bold;'>『📊 執行AI分析』</span>，AI將融合基本面與技術面指標提供交易策略。", unsafe_allow_html=True)
    st.markdown("---")
    ...
    with st.spinner(f'AI 分析引擎啟動中，正在分析 {final_symbol_name}...'):
        ...
        if data is not None and len(data) > 20: # 確保有足夠基礎數據
            df = calculate_indicators(data.copy())
            if df.empty or len(df) < 2:
                st.error("錯誤：數據不足以計算核心技術指標，請嘗試更長的分析週期。")
            else:
                ...
                expert_summary_html = generate_expert_summary(signal, risk, df, final_symbol_name)
                ...
                st.markdown(expert_summary_html, unsafe_allow_html=True)
                st.markdown("---")
                ...
                is_buy_signal = "買" in signal['action'] or "Buy" in signal['action']
                tp_color_class = 'positive' if is_buy_signal else 'negative'
                sl_color_class = 'negative' if is_buy_signal else 'positive'
                rr_text = f"{abs((risk['tp'] - risk['entry']) / (risk['entry'] - risk['sl'])):.2f}" if (risk['entry'] - risk['sl']) != 0 else "N/A"
                st.markdown(f"""
                <div class="metric-container">
                    ...
                    <div class="metric-card"><div class="metric-label">🚀 共識止盈 (TP)</div><div class="metric-value {tp_color_class}">{risk['tp']:,.2f}</div></div>
                    <div class="metric-card"><div class="metric-label">🛑 共識止損 (SL)</div><div class="metric-value {sl_color_class}">{risk['sl']:,.2f}</div></div>
                </div>
                <div class="strategy-summary">
                💡 <b>策略總結:</b> 基於 {signal['action']} 信號，建議在 <b>~{risk['entry']:,.2f}</b> 範圍內尋找機會 | ⚖️ <b>風險/回報比 (R:R):</b> {rr_text} | 波動單位 (ATR): {risk['atr']:.4f}
                </div>
                """, unsafe_allow_html=True)
                st.subheader("🧠 關鍵決策依據 (XAI)")
                opinions_html = "<table class='custom-table'><thead><tr><th>AI分析維度</th><th>判斷結果</th></tr></thead><tbody>"
                for dim, result in signal['opinions'].items(): opinions_html += f"<tr><td>{dim}</td><td>{result}</td></tr>"
                ...
                st.subheader("🧪 策略回測報告 (SMA 20/50 交叉)")
                ...
                fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.06, row_heights=[0.6, 0.2, 0.2], subplot_titles=("價格走勢", "動能指標 (MACD / RSI)", "資金流與擺盪指標 (CMF / Stoch)"), specs=[[{"secondary_y": True}], [{"secondary_y": True}], [{"secondary_y": True}]])
                fig.add_trace(go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], name='K線', increasing_line_color='#dc3545', decreasing_line_color='#28a745'), row=1, col=1, secondary_y=False)
                ...
                if 'EMA_200' in df.columns: fig.add_trace(go.Scatter(x=df.index, y=df['EMA_200'], name='EMA 200', line=dict(color='purple', width=2, dash='dash')), row=1, col=1)
                fig.add_trace(go.Scatter(x=df.index, y=df['BB_High'], name='Bollinger 高軌', line=dict(color='grey', width=1, dash='dash')), row=1, col=1)
                fig.add_trace(go.Scatter(x=df.index, y=df['BB_Low'], name='Bollinger 低軌', line=dict(color='grey', width=1, dash='dash')), row=1, col=1)
                fig.add_trace(go.Scatter(x=df.index, y=df['Ichimoku_A'], name='Ichimoku A', line=dict(color='rgba(40, 167, 69, 0.5)', width=1.5)), row=1, col=1)
                fig.add_trace(go.Scatter(x=df.index, y=df['Ichimoku_B'], name='Ichimoku B', line=dict(color='rgba(220, 53, 69, 0.5)', width=1.5), fill='tonexty', fillcolor='rgba(150, 100, 100, 0.1)'), row=1, col=1)
                ...
                fig.add_trace(go.Scatter(x=df.index, y=df['Stoch_%K'], name='Stochastic %K', line=dict(color='cyan')), row=3, col=1, secondary_y=False)
                cmf_colors = np.where(df['CMF'] > 0, '#28a745', '#28a745')
                fig.add_trace(go.Bar(x=df.index, y=df['CMF'], name='CMF', marker_color=cmf_colors, opacity=0.6), row=3, col=1, secondary_y=True)
                ...
                fig.update_yaxes(title_text="RSI/MACD", row=2, col=1, secondary_y=False)
                fig.update_yaxes(title_text="ADX", range=[0, 100], row=2, col=1, secondary_y=True, showgrid=False)
                fig.update_yaxes(title_text="Stoch %K", range=[0, 100], row=3, col=1, secondary_y=False)
                fig.update_yaxes(title_text="CMF", row=3, col=1, secondary_y=True, showgrid=False)
