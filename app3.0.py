...
專家增強最終版實作 (V6.1 - Indicator Expansion & Color Standardization)
...
核心功能：
- [擴充] 新增 OBV (能量潮) 與 Stochastic (隨機指標)，大幅提升分析深度
- [標準化] 統一所有分析表格 (AI判讀, 技術指標, 回測) 的色彩邏輯 (紅=多頭, 綠=空頭)
- [調整] 移除「最新相關新聞」區塊，讓版面更聚焦於數據分析
- [微調] 將「建議操作」中的「觀望」文字顏色調整為 #FA8072
- [升級] 將主題還原為專業深色模式
...
版本：6.1.0 (Indicator Expansion & Color Standardization)
"""
...
# (UPDATED v5.8) 全局 CSS 樣式，還原為深色主題
st.markdown("""
<style>
    /* 全局深色主題 */
    body, .stApp {
...
def generate_ai_fusion_signal(df, fa_score):
    ...
    # 新增 OBV 判讀
    if latest['OBV'] > latest['OBV_EMA']: opinions['成交量分析 (OBV)'] = "<span class='positive'>買盤強勁: 成交量確認趨勢</span>"
    else: opinions['成交量分析 (OBV)'] = "<span class='negative'>賣壓沉重: 成交量背離趨勢</span>"
...
def get_technical_data_df(df):
    ...
    # 新增 OBV
    obv_conclusion = "<span class='positive'>買盤確認</span>" if latest['OBV'] > latest['OBV_EMA'] else "<span class='negative'>賣壓確認</span>"
    data.append(['能量潮 (OBV)', f"{latest['OBV']:.0f}", obv_conclusion])
    ...
    with st.spinner(f'AI 分析引擎啟動中...'):
        ...
        if data is not None and len(data) > 50:
            ...
            if df.empty:
                ...
                action_style = ""
                if short_action_text == "觀望":
                    action_style = 'style="color: #FA8072; font-weight: bold;"'
                elif "買" in short_action_text:
                    action_style = 'class="positive"'
                elif "賣" in short_action_text:
                    action_style = 'class="negative"'
                else:
                    action_style = 'class="neutral"'
