...
def generate_ai_fusion_signal(df, fa_score):
    ...
    if body_size > 0.7 * latest['ATRr_14']:
...
with st.sidebar:
    ...
    if manual_input:
        ...
        # 嘗試從 map 中找到名稱
        try:
           final_symbol_name = [v['name'] for k, v_dict in FULL_SYMBOLS_MAP.items() for sk, v in v_dict.items() if sk == final_symbol][0]
        except IndexError:
           final_symbol_name = ""
    else:
...
if start_analysis:
    with st.spinner('AI 分析引擎啟動中，請稍候...'):
        ...
        if data is not None:
            ...
            col2.metric("共識止盈價 (TP)", f"{risk_levels['tp']:,.2f}", delta=f"{((risk_levels['tp']/data_with_indicators['Close'].iloc[-1])-1)*100:.2f}%")
            col3.metric("共識止損價 (SL)", f"{risk_levels['sl']:,.2f}", delta=f"{((risk_levels['sl']/data_with_indicators['Close'].iloc[-1])-1)*100:.2f}%", delta_color="inverse")
            ...
            with col_main:
                ...
                indicators_df = pd.DataFrame({
                    ...
                    '數值': [
                        f"{latest_indicators['RSI_14']:.2f}",
                        f"{latest_indicators['MACD_12_26_9']:.2f}",
                        f"{latest_indicators['ADX_14']:.2f}",
                        f"{latest_indicators['CMF_20']:.2f}",
                        f"{latest_indicators['BBL_20_2.0']:.1f} - {latest_indicators['BBU_20_2.0']:.1f}"
                    ],
                    'AI 判讀': [
                        "趨於超買" if latest_indicators['RSI_14'] > 65 else ("趨於超賣" if latest_indicators['RSI_14'] < 35 else "中性"),
                        "多頭" if latest_indicators['MACDh_12_26_9'] > 0 else "空頭",
                        "強趨勢" if latest_indicators['ADX_14'] > 25 else "盤整",
                        "資金流入" if latest_indicators['CMF_20'] > 0 else "資金流出",
                        "波動放大" if (latest_indicators['BBU_20_2.0'] - latest_indicators['BBL_20_2.0']) / latest_indicators['Close'] > 0.05 else "波動收斂"
                    ]
                })
